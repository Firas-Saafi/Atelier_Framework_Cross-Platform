...
class _MeteoDetailsPageState extends State<MeteoDetailsPage> {
  ...
  void getMeteoData(String ville) {
    ...
    http.get(Uri.parse(url)).then((res) {
      setState(() {
        this.meteoData = json.decode(res.body);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: meteoData == null
            ...
            ListView.builder(
          itemCount: (meteoData == null || meteoData['list'] == null
              ? 0
              : meteoData['list'].length),
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            ...
                            (DateTime.fromMicrosecondsSinceEpoch(meteoData['list'][index]['dt'] * 1000000))}",
                            style: TextStyle(
                              fontSize: 22, fontWeight: FontWeight.bold) ,
                          ),
                          Text(
                            "${new DateFormat ('HH:mm').format
                            (DateTime.fromMicrosecondsSinceEpoch(meteoData['list'][index]['dt'] * 1000000))}" " | ${meteoData['list'][index]['weather'][0]['main']}",
                            style: TextStyle(
                              fontSize: 22, fontWeight: FontWeight.bold) ,
                          ),
                        ],
